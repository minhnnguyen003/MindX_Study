    ...
    connectedCallback() {
        ...
        this.shadow.getElementById("delete").addEventListener('click', () => {
            firebase.firestore().collection("employees").doc(this.id).delete().then(() => {
                document.querySelector("main-screen").shadowRoot.getElementById(this.id).remove();
            })
        })
        ...
        this.shadow.getElementById("update").addEventListener("click", () => {
            const updateForm = document.querySelector("main-screen").shadowRoot.querySelector("custom-update");
